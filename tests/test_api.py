...
def test_create_validate_and_plan_workflow(tmp_path: Path) -> None:
    ...
    created = client.post("/workflows", json=_valid_workflow_payload(), headers=AUTH_HEADERS)
    ...
    validated = client.post(f"/workflows/{workflow_id}/validate", headers=AUTH_HEADERS)
    assert validated.status_code == 200
    assert validated.json()["valid"] is True

    planned = client.post(f"/workflows/{workflow_id}/plan", headers=AUTH_HEADERS)
...
def test_invalid_workflow_returns_bad_request_for_plan(tmp_path: Path) -> None:
    ...
    created = client.post("/workflows", json=payload, headers=AUTH_HEADERS)
...
def test_workflow_persists_across_app_instances(tmp_path: Path) -> None:
    ...
    created = first_client.post("/workflows", json=payload, headers=AUTH_HEADERS)
    assert created.status_code == 201
    workflow_id = created.json()["id"]

    second_client = TestClient(create_app(database_url=database_url))
    fetched = second_client.get(f"/workflows/{workflow_id}", headers=AUTH_HEADERS)
...
def test_workflow_versioning_and_publish_flow(tmp_path: Path) -> None:
    ...
    created = client.post(
        "/workflows",
        json=_valid_workflow_payload(name="Support triage v1"),
    ...
    latest = client.get(f"/workflows/{workflow_id}", headers=AUTH_HEADERS)
    assert latest.status_code == 200
    assert latest.json()["version"] == 2
    assert latest.json()["state"] == "draft"

    published_v1_lookup = client.get(f"/workflows/{workflow_id}?version=1", headers=AUTH_HEADERS)
...
def test_publish_rejects_invalid_draft_version(tmp_path: Path) -> None:
    ...
    publish_invalid = client.post(
        f"/workflows/{workflow_id}/versions/2/publish",
        headers=AUTH_HEADERS,
    )
